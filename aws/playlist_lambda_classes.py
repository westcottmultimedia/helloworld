...
class Playlist(object):
    ...
    def init(self):
        ...
        self.tracks = {}
        self.track_count = 0
        self.old_version = None # whatevers in db
        self.new_version = None # snapshot id from API
        self.is_version_same = None # compare old to new, function call, self.is_version_same(old, new)
        self.is_followers_processed = False
        self.is_positions_processed = False
        self.is_popularities_processed = False
...
def remove_unwanted_playlists(playlists):
    ...
    wanted = [pl for pl in playlists if pl['playlist_id'] not in unwanted]
    return wanted
...
#
def append_db_playlist_info_and_update_version(playlists):
    for playlist_id, playlist in playlists.items():
        try:
            ...
            playlists[playlist_id]['is_playlist_version_same'] = playlist_version_same
...
def refresh_playlists_of_users(users):
    ...
    playlists = append_db_playlist_info_and_update_version(playlists)
